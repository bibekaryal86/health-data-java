...
public class CheckupCategoryService {
  ...
  @Cacheable(
      value = "checkupCategories",
      unless = "#result == null || #result.checkupCategoryList.size() == 0")
  ...
  public CheckupCategoryResponse saveCheckupCategory(CheckupCategory checkupCategory) {
    ...
    try {
      ...
      checkupCategoryResponse = CheckupCategoryResponse.builder().modifiedCount(1).build();
    } catch (Exception ex) {
      log.error("Exception in Save Checkup Category", ex);
      checkupCategoryResponse =
          CheckupCategoryResponse.builder().errMsg(ConstantUtils.ERR_MSG_SAVE).build();
    }
  ...
  public CheckupCategoryResponse deleteCheckupCategory(Integer id) {
    ...
    try {
      ...
      checkupCategoryResponse =
          CheckupCategoryResponse.builder().errMsg(ConstantUtils.ERR_MSG_DELETE).build();
    }

    return checkupCategoryResponse;
  }

  public boolean isValidCheckupCategoryRequest(
      CheckupCategoryRequest checkupCategoryRequest, boolean isInsert) {
