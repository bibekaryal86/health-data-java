...
public class CheckupComponentService {
    ...
    public CheckupComponentService(CheckupComponentRepository checkupComponentRepository,
    ...
    @Cacheable(value = "checkupComponents", unless = "#result == null || #result.checkupComponentList.size() == 0")
    public CheckupComponentResponse findCheckupComponents() {
        ...
        try {
            ...
            List<CheckupCategory> checkupCategoryList = checkupCategoryService.findCheckupCategories().getCheckupCategoryList();
            List<CheckupComponent> checkupComponentList = checkupComponentDtoList.stream()
                    .map(checkupComponentDto -> convertDtoToObject(checkupComponentDto, checkupCategoryList))
                    .sorted(Comparator.comparing((CheckupComponent cc) -> cc.getCheckupCategory().getCategoryName())
                                        .thenComparing(CheckupComponent::getComponentName))
                    .collect(Collectors.toList());

            checkupComponentResponse = CheckupComponentResponse.builder()
                    .checkupComponentList(checkupComponentList)
                    .build();
        } catch (Exception ex) {
    ...
    public boolean isValidCheckupComponentRequest(CheckupComponentRequest checkupComponentRequest, boolean isInsert) {
